  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // если сервер возвращает объект вида { status: 200, transactions: [...] }
        if (transactionResponse?.status === 200 && Array.isArray(transactionResponse.transactions)) {
            setTransactionData(transformTransactionData(transactionResponse.transactions, selectedMonth, selectedYear));
        } 
        // если сервер сразу возвращает массив транзакций
        else if (Array.isArray(transactionResponse)) {
            setTransactionData(transformTransactionData(transactionResponse, selectedMonth, selectedYear));
        } else {
            setTransactionData([]); // безопасный дефолт
        }
        } catch (error) {
        setTransactionData([]); // безопасный дефолт при ошибке
        showMessage(error.response?.data?.message || "Error Loading Transactions: " + error);
        }
    };
    fetchData();
          ...
          <div className="chart-section">
            <div className="chart-container">
                ...
                <ResponsiveContainer width="100%" height={400}>
                    <LineChart data={Array.isArray(transactionData) ? transactionData : []}>
