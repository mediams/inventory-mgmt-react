...
const AddEditProductPage = () => {
  const { productId } = useParams(); // <-- без аргумента
  const [name, setName] = useState("");
  const [sku, setSku] = useState("");
  const [price, setPrice] = useState("");
  const [stockQuantity, setStockQuantity] = useState(""); // <-- исправлен setter
  ...
  useEffect(() => {
    ...
    const fetchProductById = async () => { // <-- поправил имя функции
      if (productId) {
        setIsEditing(true);
        try {
  ...
  const handleImageChange = (e) => {
    ...
    reader.onloadend = () => setImageUrl(reader.result); // preview
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <input
              type="number"
              value={stockQuantity}
              onChange={(e) => setStockQuantity(e.target.value)} // <-- исправлен setter
              required
            />
          </div>
